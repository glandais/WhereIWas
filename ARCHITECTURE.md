...
The status screen reads `TrackingStatus.appliedProfile` (the engine's `appliedProfile`), not `activeProfile`: STATIONARY reads "GPS off" (the old coarse-profile downgrade is gone — see *Battery*). `activeProfile` stays the high-accuracy profile that annotates the samples.

### Rules
* **Toward MOVING is immediate** on a credible activity: `impliesMotion` and confidence ≥ `minimumActivityConfidence` (medium). GPS speed alone is slower on purpose: it takes `movingFixConfirmations` (2) **consecutive** probing fixes at ≥ `movingSpeedThreshold` (0.7 m/s), since CoreLocation occasionally puts a large speed on a fix that has not moved. Any slower or speed-less fix breaks the streak, as does leaving PROBING. The same confirmation gates the profile while MOVING: `profileSpeed` — distinct from the raw `lastSpeed` the status screen shows — climbs a speed tier only once that many fixes agree, and falls back at the first slower fix.
* **A confirmed settle silences the `unknown` rule.** `settledStationary` is set by a `stationary` activity at high confidence **outside MOVING**, and by a probe window that expired *after seeing at least one fix*; it is cleared by a motion hint, a significant change, a visit, any activity that `impliesMotion`, any fix at or above `movingSpeedThreshold`, and by `disable`. While set, a confidently `unknown` report no longer reopens PROBING — the classifier flaps between `stationary/low` and `unknown/high` while the phone sits still, and probing on every flap woke GPS 259 times in one recorded day. Three cases are deliberately excluded, not oversights: a phone on a car seat is genuinely stationary *to CoreMotion* while the car drives on, so settling from MOVING would silence the very reports that bring tracking back; a probe window that saw **no** fix (indoors, a garage, a cold receiver) is absence of evidence, not evidence of stillness; and a measured GPS speed directly refutes the guess the settle encodes. Cost: while settled, the pedometer is the only *fast* way back and needs ten real steps (`TrackingCoordinator.motionHintStepThreshold`) — a driver who never walks depends on CoreMotion classifying `automotive`, or on a significant change (~500 m). If that ever proves too coarse, the fix is a bound on how long a settle may last, not a return to probing on every flap.
* **A trip never ends in the dark.** `recentlyMoved` is set on entering MOVING and cleared only on reaching STATIONARY; while it holds, every decision that would switch GPS off lands in SETTLING instead — a `settlingTimeout` window on a wide distance filter whose only job is to catch the departure. Nothing shortens it: a `stationary` report does not set `settledStationary` either (that would make the restart *after* the window slower than if it had never run). Only the timer reaches STATIONARY, settling on the same rule a probe window does — fixes seen and none of them moving. Without it, GPS went off the instant a stop was decided and a bicycle or car pulling away minutes later (no pedometer steps, CoreMotion slow to say `cycling`) went unseen until the next significant change; `settlingTimeout = 0` restores that. Not free: a stop pays up to five minutes of rationed GPS (a phone that never moved never pays it — `recentlyMoved` stays false through PROBING); accuracy is not the knob (iOS delivers what its receiver already has — 6 m median fixes for a `threeKilometers` request in one recorded day), so the distance filter is what rations, and the fixes it does deliver are stored like any other, so the departure is traced from its first 50 m, not from the MOVING transition that follows.
* **Toward STATIONARY is hysteretic**: from MOVING only via the stillness timer (`stillnessTimeout`, 120 s default), armed by a credible `stationary` activity or a fix slower than `stillSpeedThreshold` (0.3 m/s) and cancelled by any moving activity or a fast fix (unless the classifier currently says stationary — GPS speed jitter must not defeat CoreMotion).
* **A timer that fires must have been fed.** Arming takes one still reading; reaching STATIONARY takes a *second* one, arriving while the countdown ran (`stillnessCorroborated`, set by any further arm attempt and cleared on cancel and on each fresh arming). Without it, the timer firing says only that nothing arrived since arming — exactly what a suspended app looks like, so one stray still reading could declare STATIONARY mid-ride. Uncorroborated, `stillnessTimerFired` goes to PROBING instead — one `probeTimeout` of GPS, settling into STATIONARY anyway when nothing is moving. Same principle as `settledStationary` above: absence of evidence is not evidence of stillness.
* **Significant change / visits never jump to MOVING**: ~500 m accuracy, and they also fire when *arriving*. They open a PROBING window; a real fix decides.
* **`enable` → PROBING**: on user toggle and on every relaunch we want a first fix and speed reading immediately.
* `startGPS(profile)` is re-emitted whenever the computed profile changes while MOVING; the engine diffs against `currentProfile` and reconfigures in place — it never calls `stopUpdatingLocation`, but it *does* call `startUpdatingLocation()` on every request, unchanged profile included. That call is what registers the background location activity keeping the process alive, so it must happen on every request, not once per launch. It is idempotent while updates run. The tier comes from `GPSProfile.speedTier` — `0` slow or unknown, `1` above 2.5 m/s, `2` above 7 m/s, `3` above 12.5 m/s — which the confirmation above also compares against. The top tier carries no profile of its own (2 and 3 both mean "vehicle"); it exists so a `cycling` label crossing 12.5 m/s is a tier change the machine acts on. `profileSpeed` follows every fix the machine sees, so a drive's speed never leaks into the profile of the walk that follows.
...
profile. They keep their own profile up to `cyclingVehicleSpeedThreshold` (12.5 m/s, 45 km/h), above
which a bicycle label is wrong often enough — a bike on a car rack, a classifier that has not caught up
with the drive that just started — that speed wins again.
...
The only index in the schema is the one SwiftData creates for `@Attribute(.unique) sequence` (and the same on `AuditEventLog.id`). Nothing else is indexed: the audit trail is filtered and sorted in memory by `LocationStore.auditEvents`, affordable because its retention is a week.
...
* **tests performed** — `LocationFilter.trace(_:previous:now:settings:)` re-runs the filter recording each check (`horizontalAccuracy.withinLimit`, `timestamp.notStale`, …) with the measured value, the threshold and a verdict of passed / failed / skipped / not-applicable. `LocationFilterTraceTests` (in `WhereIWasTests/AuditTrailTests.swift`) asserts the trace and `evaluate` agree over a grid of inputs, so the trail can never describe a decision the filter did not make;
...
Recording costs nothing when off: `AuditLog.record` takes an `@autoclosure`, so a disabled trail never even builds the event, and `LocationEngine` skips `trace` entirely (`evaluate` stays on the hot path). When on, events are buffered in a 400-event ring and written off the main actor in batches of 40 (`AuditLog.ringCapacity` / `flushBatchSize`), flushed on background and at once on the rows that mark where execution stood — state transitions, heartbeats, launches (`AuditLog.flushesAtOnce`). Without that, a process killed during a quiet phase took its part-filled batch with it, and the trail read as an app that died on its last flushed row.
...
The trail has its own switches and its own retention (`auditRetentionDays`, 7 days by default, purged by the same `BGProcessingTask`), because it turns over much faster than the samples. `AuditExporter` writes it as JSON (with the settings in force, so a reader knows what was being recorded) or plain text, **gzipped by default** — a week of trail is tens of megabytes of text and deflates about sixteen to one, the difference between a share sheet that works and one that gives up. `AuditFileWriter` compresses on the way out, so the uncompressed file never exists on disk and nothing beyond one page is held in memory; `Compression`'s `COMPRESSION_ZLIB` is raw DEFLATE, so the gzip container (ten-byte header, CRC-32 and size trailer) is written by hand, and tests check it against the standard CRC value and inflate it back rather than against the writer itself. `AuditLogView` reads it back, filtered by category and minimum severity.
...
* **Stationary for long**: while STATIONARY we stop GPS by design; if the process is later terminated we depend on significant change / visit to relaunch. First movement after a long stop may lose up to ~500 m / a few minutes before PROBING starts, but only after the SETTLING window has expired (a departure within `settlingTimeout` of the stop is caught by a fix). The held `CLBackgroundActivitySession`, not location updates, is what keeps the process alive across a stop, so CoreMotion callbacks usually still arrive and the gap is small.
* **The system location indicator** (the blue pill around the clock) is on for as long as tracking is enabled — the session is held throughout, and `showsBackgroundLocationIndicator` is forced true because hiding it is a documented way for iOS to suspend the app in the background. There is no setting for it: `Formatting` still translates the now-unused `indicator.shown` / `indicator.hidden` / `indicator.forced` codes, for trails written before the removal.
...
* **Motion permission denied**: the state machine only has GPS speed and significant change to work with. Coordinator falls back to a longer PROBING (`probeTimeout` × 2) — GPS is used more, battery suffers, tracking still works. It used to force coarse stationary updates on as well; that fallback went with the profile.
...
* `App/ScreenshotMode.swift` reads `-screenshotMode` / `-screenshotScreen` / `-screenshotScenario`
  ...
  launch per capture, landing straight on the right tab.
...
* A third task, the **heartbeat**, writes `app.heartbeat` every 30 s while the phase is GPS-active *and* the trail is on. It records nothing about tracking, only that the process was executing — without it an export cannot separate "the receiver reported nothing" from "the app was suspended" (a 120 s stillness timer once fired 1224 s after arming, with not one trail row in between). `app.background` / `app.foreground` and CoreLocation's own `location.updates.paused` / `.resumed` (recorded even though `pausesLocationUpdatesAutomatically` is false, precisely because the assumption is what is in doubt) frame the same gaps. The JSON and text exports name the build that wrote them.
...
3. Termination: with tracking on, run `xcrun devicectl device process terminate` (or let iOS kill it by opening a few heavy apps), then move > 500 m. The app must relaunch (Console.app filter `subsystem:io.github.glandais.whereiwas`) and an `enable/relaunch` transition appears.
...
6. Battery: Settings › Battery after a full day.
