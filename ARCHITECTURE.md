...
The status screen reads `TrackingStatus.appliedProfile` (the engine's `appliedProfile`), not `activeProfile`. The two used to differ: STATIONARY downgraded to a coarse profile rather than stopping, so the row read "Stationary (coarse)". That profile is gone — see the note under *Battery* — and STATIONARY now reads "GPS off". `activeProfile` stays the high-accuracy profile, which is what annotates the samples.
...
* `startGPS(profile)` is re-emitted whenever the computed profile changes while MOVING (speed tier or activity changed); the engine diffs against `currentProfile` and reconfigures the manager in place — it never calls `stopUpdatingLocation`, but it *does* call `startUpdatingLocation()` on every request, unchanged profile included. That call is not only how updates begin: it is what registers the background location activity keeping the process alive, and calling it once per launch is how a ride came back as 19 fixes for 5.5 km, the app woken every five minutes by a significant change and suspended ten seconds later. It is idempotent while updates run. The tier comes from `GPSProfile.speedTier` — `0` slow or unknown, `1` above 2.5 m/s, `2` above 7 m/s, `3` above 12.5 m/s — which is also what the confirmation above compares. The top tier carries no profile of its own for a slow label (2 and 3 both mean "vehicle" there); it exists so that a `cycling` label crossing 12.5 m/s, which *does* change the profile, is a tier change the machine acts on. `profileSpeed` follows every fix the machine sees, so a drive's speed never leaks into the profile of the walk that follows.
...
The trail has its own switches and its own retention (`auditRetentionDays`, 7 days by default, purged by the same `BGProcessingTask`), because it turns over much faster than the samples. `AuditExporter` writes it as JSON (with the settings in force, so a reader knows what was being recorded) or plain text, **gzipped by default** — a week of trail is tens of megabytes of text and deflates by about sixteen to one, which is the difference between a share sheet that works and one that gives up. `AuditFileWriter` compresses on the way out, so the uncompressed file never exists on disk and nothing beyond one page is held in memory; `Compression`'s `COMPRESSION_ZLIB` is raw DEFLATE, so the gzip container — ten-byte header, CRC-32 and size trailer — is written by hand, and the tests check it against the standard CRC check value and inflate it back rather than against the writer itself. `AuditLogView` reads it back, filtered by category and minimum severity.
...
* A third task, the **heartbeat**, writes `app.heartbeat` every 30 s while the phase is GPS-active *and* the trail is on. It records nothing about tracking; it records that the process was executing. Without it an export cannot separate "the receiver reported nothing" from "the app was suspended", which is the question every background investigation has run into — and it is measurable: the stillness timer armed at 13:16:05 on 2026-09-07 for 120 s fired 1224 s later, and the trail held not one row in between. `app.background` / `app.foreground` and CoreLocation's own `location.updates.paused` / `.resumed` (recorded even though `pausesLocationUpdatesAutomatically` is false, precisely because the assumption is what is in doubt) frame the same gaps. The JSON and text exports name the build that wrote them.
