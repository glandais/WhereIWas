...
While the app is stationary it records nothing. It keeps a 3 km-accuracy update stream open, which costs almost no power and is what lets iOS keep the process alive between movements — but that stream also lights the blue indicator while nothing is being written. Settings therefore has a "Show location indicator" switch, on by default, which sets showsBackgroundLocationIndicator = false for that idle case alone. It cannot hide recording: the CLBackgroundActivitySession held while the receiver is on shows the indicator whatever the switch says, and the Status screen names which of the two situations the app is in — "Stationary (coarse)" when it is only keeping the process alive, or the GPS profile in force when it is recording.
