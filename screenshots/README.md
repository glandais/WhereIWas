...
Generated by `./scripts/screenshots.sh` — five screens × nine locales, from mocked data, no device
...
`cs` — where the bundle carries `de`, `es`, `nl`. `apple_locale_for` in the script maps the store
code onto the `-AppleLocale` the simulator wants (`it` → `it_IT`, `ja` → `ja_JP`, …), so dates and
numbers come out of the market the screenshot is for.
